...
const RouterTab = {
  ...
  data() {
    return {
      ...
      hasSplitDropped: false,
      splitDragIndex: null,
      currentDraggledIframe: null
    }
  },
  ...
  mounted() {
    //修复slipt初始不能自适应问题
    document.getElementById('splitArea').style.width = 'calc(100% - 8px)'
  },
  destroyed() {
  ...
  methods: {
    changeBackground(e) {
      e.target.style.background = 'rgba(220,224,223,0.61)'
    },
    resetBackground(e) {
    ...
    onsplitDrop(e) {
      e.target.style.background = ''
      this.splitDragIndex = e.dataTransfer.getData('text') || '0'
      // 拖拽的iframe url
      console.log(this.splitDragIndex)
      console.log(this.tabs)
      console.log(this.items)
      this.currentDraggledIframe = this.iframes[this.splitDragIndex]
      this.hasSplitDropped = true
    },
    // RouterAlive 组件就绪
    onAliveReady($alive) {
      // 获取 keepAlive 组件实例
    ...
    getRouteTab({ key, $route, meta }) {
      ...
      // 支持根据路由函数返回的页签属性
      const props = ['title', 'tips', 'icon', 'closable', 'src']
